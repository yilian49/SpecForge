...
class GeneralParser(Parser):
    ...
    def parse(
        self, source: "Conversation", max_length: int
    ) -> Dict[str, List[torch.Tensor]]:
        global CURRENT_WARNINGS, MAX_WARNINGS
        messages = []
        ...
        for j, sentence in enumerate(source):
            role = sentence["role"]
            # assert role == convroles[j % 2], f"unexpected role {role}, suppose to be {convroles[j % 2]}, source is {source}"

...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Eagle3 embedding layer with offline data")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory containing the checkpoint with config.json, model.safetensors, and training_state.pt",
    )
    parser.add_argument(
        "--embedding-key",
        type=str,
...
def main():
    ...
    if draft_model_last_checkpoint:
        ...
        # Load from the provided checkpoint directory
        draft_model = (
            AutoEagle3DraftModel.from_pretrained(
                args.checkpoint_dir,
                attention_backend=args.draft_attention_backend,
    ...
    # Only enable gradients for embedding layer
    if hasattr(draft_model, 'embed_tokens'):
        draft_model.embed_tokens.weight.requires_grad = True
        print_with_rank("Enabled gradients for embed_tokens layer")
    else:
        print_with_rank("Warning: embed_tokens not found in draft model")
    
    # Load embedding from target model if needed
    if not hasattr(draft_model_config, "target_hidden_size") or draft_model_config.target_hidden_size == draft_model_config.hidden_size:
        ...
        # Don't freeze embedding since we want to train it
        if hasattr(draft_model, 'embed_tokens'):
            draft_model.embed_tokens.weight.requires_grad = True
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch % args.save_interval == 0:
            # Save the model including embedding layer
            ...
            with FSDP.state_dict_type(eagle3_model, StateDictType.FULL_STATE_DICT):
                ...
                if dist.get_rank() == 0:
                    ...
                    print_on_rank0(f"Saved model (including embeddings) to {epoch_output_dir}")
                dist.barrier()
